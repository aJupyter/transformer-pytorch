...
class SparseMOE(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f"flat_gating_output:\n{flat_gating_output} \n flat_gating_output size:\n{flat_gating_output.size()}")
        ...
        for i, expert in enumerate(self.experts):
            ...
            if flat_mask.any():
                ...
                print(f"expert_input:\n{expert_input}", expert_input.size())
                expert_output = expert(expert_input)
                print(f"expert_output:\n{expert_output}", expert_output.size())

                # Extract and apply gating scores
                gating_score = flat_gating_output[flat_mask, i].unsqueeze(1)
                print(f"gating_score:\n{gating_score}", gating_score.size())
                weight_output = expert_output * gating_score
                print(f"weight_output:\n{weight_output}", weight_output.size())

                # Update final output additively by indexing and adding
                final_output[expert_mask] += weight_output.squeeze(1)
                print(f"weight_output:\n{weight_output}")
                test = weight_output.squeeze(1)
                print(f"weight_output.squeeze(-1):\n{test}")
                print(f"final_output:\n{final_output}", final_output.size())
